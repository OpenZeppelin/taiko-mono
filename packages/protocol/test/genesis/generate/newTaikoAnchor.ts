...
// deployTaikoAnchor generates a L2 genesis alloc of the TaikoAnchor contract and CommitmentStore.
export async function deployNewTaikoAnchor(
    config: Config,
    result: Result,
): Promise<Result> {
...
async function generateContractConfigs(
    ...
    const contractArtifacts: any = {
        ...
        // Rollup Contracts - No more proxy for TaikoAnchor
        TaikoAnchor: require(
            path.join("../../../out/TaikoAnchor.sol/TaikoAnchor.json"),
        ),
        CommitmentStore: require(
            path.join("../../../out/CommitmentStore.sol/CommitmentStore.json"),
        ),
    ...
    // Shared Contracts (still using proxies)
    ...
    // Calculate initial circular blocks hash
    const initialCircularBlocksHash = ethers.utils.solidityKeccak256(
        ["bytes32[256]"],
        [new Array(256).fill(ethers.constants.HashZero)],
    );
    ...
    return {
            ...
            variables: {
                ...
                __addresses: {
                    [chainId]: {
                        ...
                        [ethers.utils.hexlify(
                            ethers.utils.toUtf8Bytes("commitment_store"),
                        )]: addressMap.CommitmentStore,
                    },
                },
            },
            slots: {
        ...
        // CommitmentStore - New contract
        CommitmentStore: {
            address: addressMap.CommitmentStore,
            deployedBytecode:
                contractArtifacts.CommitmentStore.deployedBytecode.object,
            variables: {
                // Empty storage for now, commitments will be added via storeCommitment
                _commitments: {},
            },
        },
        // TaikoAnchor - No longer a proxy, direct deployment with constructor args
        TaikoAnchor: {
            address: addressMap.TaikoAnchor,
            deployedBytecode: linkContractLibs(
                replaceImmutableValues(contractArtifacts.TaikoAnchor, [
                    {
                        id: taikoAnchorReferencesMap.fixedBaseFee.id,
                        value: ethers.utils.hexZeroPad(
                            ethers.utils.hexlify(fixedBaseFee),
                            32,
                        ),
                    },
                    {
                        id: taikoAnchorReferencesMap.permittedSender.id,
                        value: ethers.utils.hexZeroPad(permittedSender, 32),
                    },
                    {
                        id: taikoAnchorReferencesMap.commitmentStore.id,
                        value: ethers.utils.hexZeroPad(
                            addressMap.CommitmentStore,
                            32,
                        ),
            ...
            variables: {
                ...
                lastAnchorBlockId: 0, // Will be set to block.number - 1 in constructor
                lastPublicationId: 0,
                circularBlocksHash: initialCircularBlocksHash,
                blockHashes: {
                    // Will be initialized with parent block hash in constructor
                    // We'll need to calculate the correct slot for this mapping
                },
            },
        },
        RollupResolverImpl: {
...
function getImmutableReference(
    ...
    let artifact: any;
    if (contractName === "CommitmentStore" || contractName === "TaikoAnchor") {
        if (contractName === "CommitmentStore") {
            artifact = require(
                path.join(
                    `../../../out/CommitmentStore.sol/CommitmentStore.json`,
                ),
            );
        } else {
            artifact = require(
