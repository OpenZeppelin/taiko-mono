...
interface IProverManager {
    ...
    event ProverEvicted(
        address indexed prover, address indexed evictor, uint256 periodEnd, uint256 livenessBond
    );
    ...
    /// @notice If there is no active prover, start a new period and become the new prover in the
    /// next block
    ...
    /// This function allows anyone to start a new period with a new fee. Note that the new prover
    /// will still need to
    // ensure the previous publications are proven before proving their own. Nevertheless, we start
    // a new period so
    ...
    /// @param publicationHeader The publication header that the caller is claiming is too old and
    /// hasn't been proven
    function evictProver(IInbox.PublicationHeader calldata publicationHeader) external;

    /// @notice Submits a proof.
    /// @dev If called after the period has passed its proving deadline, the caller becomes the
    /// prover for the
    ...
    /// @param firstPub The first publication header in the transition. Note that since checkpoints
    /// refer to the
    ...
    /// @param numDelayedPublications The number of delayed publications from the total number of
    /// publications being
    /// proven
    /// @param proof Arbitrary data passed to the `verifier` contract to confirm the transition
    /// validity
    ...
    /// @param provenPublication A publication that the caller is claiming has been proven and is
    /// after the period end
    /// @dev If there is a proven publication after the period, it implies the whole period has been
    /// proven.
    /// @dev We assume there will always be a suitable proven publication.
    function finalizePastPeriod(
        uint256 periodId,
        IInbox.PublicationHeader calldata provenPublication
    )
        external;
}
